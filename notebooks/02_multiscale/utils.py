...
def createSR(num_shots, num_sources_per_shot, num_receivers_per_shot, num_dims,source_spacing, receiver_spacing,source_depth,receiver_depth):
    ...
    """    
    x_s = torch.zeros(num_shots, num_sources_per_shot, num_dims,dtype=torch.long)
    # if source_depth != 0:
    #     x_s[:, 0, 0] = source_depth        
    # x_s[:, 0, 1] = torch.arange(1,num_shots+1).float() * source_spacing
    x_s[..., 1]=source_depth
    x_s[:, 0, 0]=(torch.arange(num_shots) * source_spacing )
    #=========receiver location=============#
...
def loadtruemodel(data_dir, num_dims, vmodel_dim,device):
    """
        Load the true model
    """
